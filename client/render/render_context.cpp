...
bool RenderContext::init(int width, int height, const std::string& title) {
    ...
    // Request OpenGL 4.1 Core Profile (macOS max supported version)
    ...
    SDL_GL_SetSwapInterval(1); // VSync (double buffer default)
...
void RenderContext::set_vsync_mode(int mode) {
    // 0 = off (no vsync), 1 = double buffer (vsync on), 2 = triple buffer (adaptive vsync)
    // SDL_GL_SetSwapInterval: 0 = immediate, 1 = vsync, -1 = adaptive vsync (triple buffer)
    int swap_interval = 0;
    switch (mode) {
        case 0: swap_interval = 0; break;   // No VSync
        case 1: swap_interval = 1; break;   // Double buffer (VSync)
        case 2: swap_interval = -1; break;  // Triple buffer (adaptive VSync)
        default: swap_interval = 1; break;
    }
    
    // Try to set the requested mode
    if (!SDL_GL_SetSwapInterval(swap_interval)) {
        // If adaptive vsync (-1) fails, fall back to regular vsync
        if (swap_interval == -1) {
