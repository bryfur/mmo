...
inline float calculate_scale_for_model(const std::string& model_name, float target_size, 
    ...
    return (target_size * 1.5f) / fallback_max_dim;
}
...
inline void get_environment_collision_size(EnvironmentType type, float scale,
    ...
    if (bounds) {
        ...
        // Use smaller collision (0.35) for better gameplay feel around rocks/trees
        half_x = (bounds->width() * scale) * 0.35f;
        half_y = (bounds->height() * scale) * 0.5f;
        half_z = (bounds->depth() * scale) * 0.35f;
    } else {
        // Fallback
        half_x = scale * 0.3f;
        half_y = scale * 0.5f;
