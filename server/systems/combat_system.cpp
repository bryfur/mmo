...
void update_combat(entt::registry& registry, float dt) {
    ...
    for (auto entity : player_view) {
        ...
        // Determine attack cone based on class
        float cone_angle = 0.5f;  // ~60 degrees half-angle default
        switch (info.player_class) {
            case PlayerClass::Warrior:
                cone_angle = 1.1f;  // ~126 degrees total (wide arc)
                break;
            case PlayerClass::Mage:
                cone_angle = 0.2f;  // Narrow beam
                break;
            case PlayerClass::Paladin:
                cone_angle = 3.14159f;  // Full circle AOE
                break;
            case PlayerClass::Archer:
                cone_angle = 0.15f;  // Very narrow precision shot
                break;
        }
        ...
        for (auto target : targets) {
            apply_damage(registry, target, combat.damage);
        }
    }
