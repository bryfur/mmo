...
engine::CurveType parse_curve_type(const std::string& str) {
    ...
    if (str == "linear") {
        return engine::CurveType::LINEAR;
    }
    if (str == "ease_in") {
        return engine::CurveType::EASE_IN;
    }
    if (str == "ease_out") {
        return engine::CurveType::EASE_OUT;
    }
    if (str == "ease_in_out") {
        return engine::CurveType::EASE_IN_OUT;
    }
    if (str == "fade_out_late") {
        return engine::CurveType::FADE_OUT_LATE;
    }
    return engine::CurveType::CONSTANT;
}
...
engine::SpawnMode parse_spawn_mode(const std::string& str) {
    ...
    if (str == "continuous") {
        return engine::SpawnMode::CONTINUOUS;
    }
    return engine::SpawnMode::BURST;
}
...
engine::VelocityType parse_velocity_type(const std::string& str) {
    ...
    if (str == "radial") {
        return engine::VelocityType::RADIAL;
    }
    if (str == "orbital") {
        return engine::VelocityType::ORBITAL;
    }
    if (str == "custom") {
        return engine::VelocityType::CUSTOM;
    }
    if (str == "arc") {
        return engine::VelocityType::ARC;
    }
    return engine::VelocityType::DIRECTIONAL;
}
...
engine::Curve parse_curve(const json& j) {
    ...
    if (j.is_string()) {
        ...
        if (j.contains("start")) {
            curve.start_value = j["start"].get<float>();
        }
        if (j.contains("end")) {
            curve.end_value = j["end"].get<float>();
        }
        if (j.contains("fade_start")) {
            curve.fade_start = j["fade_start"].get<float>();
        }
    } else if (j.is_number()) {
...
engine::VelocityDefinition parse_velocity(const json& j) {
    ...
    if (j.contains("speed")) {
        vel.speed = j["speed"].get<float>();
    }
    if (j.contains("direction")) {
        vel.direction = parse_vec3(j["direction"], {1, 0, 0});
    }
    if (j.contains("spread_angle")) {
        vel.spread_angle = j["spread_angle"].get<float>();
    }
    if (j.contains("gravity")) {
        vel.gravity = parse_vec3(j["gravity"]);
    }
    if (j.contains("drag")) {
        vel.drag = j["drag"].get<float>();
    }
    if (j.contains("orbit_radius")) {
        vel.orbit_radius = j["orbit_radius"].get<float>();
    }
    if (j.contains("orbit_speed")) {
        vel.orbit_speed = j["orbit_speed"].get<float>();
    }
    if (j.contains("orbit_height_base")) {
        vel.orbit_height_base = j["orbit_height_base"].get<float>();
    }
    if (j.contains("height_variation")) {
        vel.height_variation = j["height_variation"].get<float>();
    }
    if (j.contains("arc_radius")) {
        vel.arc_radius = j["arc_radius"].get<float>();
    }
    if (j.contains("arc_height_base")) {
        vel.arc_height_base = j["arc_height_base"].get<float>();
    }
    if (j.contains("arc_height_amplitude")) {
        vel.arc_height_amplitude = j["arc_height_amplitude"].get<float>();
    }
    if (j.contains("arc_tilt_amplitude")) {
        vel.arc_tilt_amplitude = j["arc_tilt_amplitude"].get<float>();
    }
...
engine::RotationDefinition parse_rotation(const json& j) {
    ...
    if (j.contains("initial")) {
        rot.initial_rotation = parse_vec3(j["initial"]);
    }
    if (j.contains("rate")) {
        rot.rotation_rate = parse_vec3(j["rate"]);
    }
    if (j.contains("face_velocity")) {
        rot.face_velocity = j["face_velocity"].get<bool>();
    }
...
engine::EmitterDefinition parse_emitter(const json& j) {
    ...
    if (j.contains("name")) {
        emitter.name = j["name"].get<std::string>();
    }
    if (j.contains("particle_type")) {
        emitter.particle_type = j["particle_type"].get<std::string>();
    }
    if (j.contains("model")) {
        emitter.model = j["model"].get<std::string>();
    }

    if (j.contains("spawn_mode")) {
        emitter.spawn_mode = parse_spawn_mode(j["spawn_mode"].get<std::string>());
    }
    if (j.contains("spawn_count")) {
        emitter.spawn_count = j["spawn_count"].get<int>();
    }
    if (j.contains("spawn_rate")) {
        emitter.spawn_rate = j["spawn_rate"].get<float>();
    }

    if (j.contains("lifetime")) {
        emitter.particle_lifetime = j["lifetime"].get<float>();
    }
    ...
    if (j.contains("delay")) {
        emitter.delay = j["delay"].get<float>();
    }
    if (j.contains("duration")) {
        emitter.duration = j["duration"].get<float>();
    }
...
bool EffectRegistry::load_effect(const std::string& file_path) {
    try {
        ...
        if (!file.is_open()) {
            std::cerr << "Failed to open effect file: " << file_path << '\n';
            return false;
        }
        ...
        std::cout << "Loaded effect: " << effect_name << " from " << file_path << '\n';
        return true;

    } catch (const std::exception& e) {
        std::cerr << "Error loading effect from " << file_path << ": " << e.what() << '\n';
        return false;
    }
}
...
bool EffectRegistry::load_effects_directory(const std::string& directory_path) {
    if (!fs::exists(directory_path) || !fs::is_directory(directory_path)) {
        std::cerr << "Effects directory does not exist: " << directory_path << '\n';
        return false;
    }
