...
void build_damage_numbers(UIScene& ui, const HUDState& state, const glm::mat4& view_projection, float screen_w,
                          float screen_h) {
    for (const auto& dn : state.damage_numbers) {
        auto sp = world_to_screen(view_projection, glm::vec3(dn.x, dn.y, dn.z), screen_w, screen_h);
        if (!sp) {
            continue;
        }
...
void build_notifications(UIScene& ui, const HUDState& state, float screen_w) {
    ...
    for (const auto& notif : state.notifications) {
        ...
        const uint8_t alpha = static_cast<uint8_t>(255.0f * alpha_ratio);
        const uint32_t color = (notif.color & 0x00FFFFFFu) | (static_cast<uint32_t>(alpha) << 24);
        const uint32_t bg_alpha = static_cast<uint32_t>(alpha * 0.6f) << 24;

        const float text_w = static_cast<float>(notif.text.size()) * 8.0f;
        ui.add_filled_rect(cx - text_w * 0.5f - 10.0f, y - 5.0f, text_w + 20.0f, 28.0f, bg_alpha);
