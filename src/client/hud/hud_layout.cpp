...
float minimap_area_pixel_radius(float world_radius_units, float minimap_world_radius,
                                float minimap_pixel_radius, float min_pixels) {
    if (minimap_world_radius <= 0.0f) return min_pixels;
...
uint32_t health_bar_color(float ratio) {
    ...
    if (ratio < 0.3f) return 0xFF0000CC; // red
    if (ratio < 0.6f) return 0xFF00AAFF; // orange
...
float linear_fade(float timer, float fade_duration) {
    ...
    if (fade_duration <= 0.0f) return timer > 0.0f ? 1.0f : 0.0f;
    if (timer >= fade_duration) return 1.0f;
