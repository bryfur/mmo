...
void build_quest_log_panel(UIScene& ui, const HUDState& hud,
                           const PanelState& panel, MouseUI& mui,
                           float screen_w, float screen_h) {
    constexpr float w = 400.0f;
    constexpr float h = 400.0f;
    auto pos = mui.default_pos(WidgetId::TitleQuestLog,
    ...
    mui.push_region(WidgetId::TitleQuestLog, WidgetId::TitleQuestLog,
                    px, py, w - close_size - 8.0f, 28.0f);
    mui.push_region(WidgetId::CloseQuestLog, WidgetId::TitleQuestLog,
                    close_x, close_y, close_size, close_size);
    ui.add_text("[L/ESC] Close  [X] Abandon",
                px + 10.0f, py + h - 22.0f, 0.6f, TEXT_HINT);
    ...
    for (int i = 0; i < static_cast<int>(hud.tracked_quests.size()); ++i) {
        ...
        const float entry_h = 20.0f
            + static_cast<float>(quest.objectives.size()) * 16.0f + 8.0f;
        ...
        ui.add_text(quest.quest_name, px + 15.0f, qy + 2.0f, 0.9f,
                    selected ? 0xFFFFCC00 : WHITE);
        qy += 22.0f;

        for (const auto& obj : quest.objectives) {
            char progress[80];
            std::snprintf(progress, sizeof(progress), "  - %s: %d/%d",
                          obj.description.c_str(), obj.current, obj.required);
            ui.add_text(progress, px + 20.0f, qy, 0.7f,
                        obj.complete ? 0xFF00FF00 : 0xFFCCCCCC);
            qy += 16.0f;
        }
        qy += 8.0f;
    }
