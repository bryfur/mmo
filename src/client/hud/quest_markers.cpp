...
void build_quest_markers(UIScene& ui, const entt::registry& registry,
                         const QuestMarkerInputs& in) {
    const float radius_sq = in.visibility_radius * in.visibility_radius;

    auto npc_view = registry.view<const ecs::NetworkId, const ecs::Transform,
                                  const ecs::EntityInfo>();
    for (auto entity : npc_view) {
        ...
        auto sp = world_to_screen(in.view_projection,
                                  glm::vec3(tf.x, marker_world_y, tf.z),
                                  in.screen_w, in.screen_h);
        if (!sp) continue;

        // Cull off-screen with a generous margin so half-clipped markers
        // don't pop in/out at the edge.
        if (sp->x < -50.0f || sp->x > in.screen_w + 50.0f ||
            sp->y < -50.0f || sp->y > in.screen_h + 50.0f) continue;
        ...
        if (has_turnin) {
            ...
            ui.add_text("?", sp->x - 5.0f * scale, sp->y - 10.0f * scale,
                        1.4f * scale, 0xFF00FF00);
        } else {
            ...
            ui.add_text("!", sp->x - 4.0f * scale, sp->y - 10.0f * scale,
                        1.4f * scale, 0xFF00DDFF);
        }
    }
}
