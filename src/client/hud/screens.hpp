...
void build_class_select(engine::scene::UIScene& ui,
                        const std::vector<protocol::ClassInfo>& classes,
                        int selected_index,
                        float screen_w, float screen_h);

void build_connecting(engine::scene::UIScene& ui,
...
void build_player_health_bar(engine::scene::UIScene& ui,
                             float current, float max,
                             float screen_w, float screen_h);

// Full-screen "YOU DIED — press SPACE to respawn" overlay.
void build_death_overlay(engine::scene::UIScene& ui,
                         float screen_w, float screen_h);
