...
TalentLayoutMetrics measure_talent_tree(const PanelState& panel) {
    ...
    for (int i = 0; i < static_cast<int>(panel.talent_tree.size()); ++i) {
        ...
        if (t.branch_name != prev_branch) {
            if (!prev_branch.empty()) {
                m.total_h += 6.0f;
            }
            m.total_h += 18.0f;
            prev_branch = t.branch_name;
        }
        if (i == panel.talent_cursor) {
            m.selected_y_start = m.total_h;
        }
...
void update_scroll(PanelState& panel, const TalentLayoutMetrics& m, float view_h) {
    ...
    if (m.selected_y_start - scroll < 0.0f) {
        scroll = m.selected_y_start;
    }
    if (selected_end - scroll > view_h) {
        scroll = selected_end - view_h;
    }
    if (scroll < 0.0f) {
        scroll = 0.0f;
    }
...
void build_talent_panel(UIScene& ui, PanelState& panel, MouseUI& mui, float screen_w, float screen_h) {
    constexpr float w = 420.0f;
    float h = std::clamp(screen_h * 0.8f, 400.0f, 700.0f);
    auto pos = mui.default_pos(WidgetId::TitleTalents, (screen_w - w) * 0.5f, (screen_h - h) * 0.5f);
    ...
    mui.push_region(WidgetId::TitleTalents, WidgetId::TitleTalents, px, py, w - close_size - 8.0f - 110.0f, 28.0f);
    mui.push_region(WidgetId::CloseTalents, WidgetId::TitleTalents, close_x, close_y, close_size, close_size);

    char pts[32];
    std::snprintf(pts, sizeof(pts), "Points: %d", panel.talent_points);
    ui.add_text(pts, px + w - 100.0f, py + 8.0f, 0.7f, panel.talent_points > 0 ? 0xFF00FF00 : 0xFFFFCC00);

    // Footer hint
    ui.add_filled_rect(px, py + h - 28.0f, w, 28.0f, 0xE0222222);
    ui.add_text("[W/S] Navigate  [Enter] Unlock  [T/ESC] Close", px + 10.0f, py + h - 22.0f, 0.55f, TEXT_HINT);
    ...
    for (int i = 0; i < static_cast<int>(panel.talent_tree.size()); ++i) {
        ...
        if (slot_y + row_h > content_top && slot_y < content_bottom) {
            ...
            const bool prereq_met = talent.prerequisite.empty() || talent_unlocked(panel, talent.prerequisite);

            ui.add_filled_rect(px + 10.0f, slot_y, w - 20.0f, row_h, selected ? 0x40FFFFFF : 0x15FFFFFF);
            ...
            if (selected) {
                ui.add_text_wrapped(talent.description, px + 50.0f, slot_y + 18.0f, w - 100.0f, 0.5f, 0xFFAAAAAA);
            }

            // Click selects the row; Enter (or a 2nd click) actually unlocks —
            // keeps destructive actions deliberate.
            mui.push_region(talent_row_id(i), WidgetId::TitleTalents, px + 10.0f, slot_y, w - 20.0f, row_h);
        }
