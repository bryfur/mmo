...
void build_vendor_window(UIScene& ui, const HUDState& hud, MouseUI& mui, float screen_w, float screen_h) {
    ...
    mui.push_region(WidgetId::TitleVendor, WidgetId::TitleVendor, x, y, w - close_size - 8.0f, 26.0f);
    mui.push_region(WidgetId::CloseVendor, WidgetId::TitleVendor, close_x, close_y, close_size, close_size);
    ...
    mui.push_region(WidgetId::VendorTab, WidgetId::TitleVendor, x + 10, tab_y - 2, 190.0f, 18.0f);
    ...
    for (int i = 0; i < count && i < max_rows; ++i) {
        ...
        const bool hovered = mui.mouse_x >= x + 10 && mui.mouse_x <= x + w - 10 && mui.mouse_y >= row_y &&
                             mui.mouse_y <= row_y + row_h - 2;
        const uint32_t row_color = selected ? 0xFF334466 : (hovered ? 0xFF2A3A4A : 0xFF222233);
        ui.add_filled_rect(x + 10, row_y, w - 20, row_h - 2, row_color);

        const auto& slot = v.stock[i];
        uint32_t name_color = 0xFFCCCCCC;
        if (slot.rarity == "uncommon") {
            name_color = 0xFF00CC00;
        } else if (slot.rarity == "rare") {
            name_color = 0xFF0088FF;
        } else if (slot.rarity == "epic") {
            name_color = 0xFFCC00CC;
        } else if (slot.rarity == "legendary") {
            name_color = 0xFF00AAFF;
        }
        ...
        if (slot.stock < 0) {
            ...
            std::snprintf(buf, sizeof(buf), "%s (stock: %d)", slot.item_name.c_str(), slot.stock);
        }
        ...
        mui.push_region(vendor_row_id(i), WidgetId::TitleVendor, x + 10, row_y, w - 20, row_h - 2);
    }

    ui.add_text("Click to purchase  -  Drag title to move  -  Esc to close", x + 12, y + h - 22, 0.7f, 0xFFAAAAAA);
}
