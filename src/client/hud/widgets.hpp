...
// Bottom-right shortcut bar: clickable buttons for each panel/menu.
// Highlights the button matching the currently-open panel (if any).
void build_menu_bar(engine::scene::UIScene& ui, const PanelState& panel, MouseUI& mui, float screen_w, float screen_h);
