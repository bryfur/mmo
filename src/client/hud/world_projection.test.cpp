...
glm::mat4 default_vp() {
    const glm::mat4 view = glm::lookAt(glm::vec3(0.0f, 0.0f, 0.0f),  // eye
                                       glm::vec3(0.0f, 0.0f, -1.0f), // forward
                                       glm::vec3(0.0f, 1.0f, 0.0f)); // up
    const glm::mat4 proj = glm::perspective(glm::radians(60.0f), kScreenW / kScreenH, 0.1f, 1000.0f);
    return proj * view;
}

TEST(WorldToScreen, PointInFrontProjectsOnScreen) {
    auto p = world_to_screen(default_vp(), glm::vec3(0.0f, 0.0f, -10.0f), kScreenW, kScreenH);
...
TEST(WorldToScreen, RightOfCameraGoesRightOnScreen) {
    auto p = world_to_screen(default_vp(), glm::vec3(2.0f, 0.0f, -10.0f), kScreenW, kScreenH);
    ASSERT_TRUE(p.has_value());
...
TEST(WorldToScreen, AboveCameraGoesUpOnScreen) {
    ...
    auto p = world_to_screen(default_vp(), glm::vec3(0.0f, 2.0f, -10.0f), kScreenW, kScreenH);
    ASSERT_TRUE(p.has_value());
...
TEST(WorldToScreen, FartherPointsConverge) {
    ...
    auto near_p = world_to_screen(default_vp(), glm::vec3(2.0f, 0.0f, -5.0f), kScreenW, kScreenH);
    auto far_p = world_to_screen(default_vp(), glm::vec3(2.0f, 0.0f, -50.0f), kScreenW, kScreenH);
