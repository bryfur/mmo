 ...
 * All rendering uses SDL3 GPU pipelines.
 */
class Renderer {
public:
    Renderer();
    ...
    bool init(int width, int height, const std::string& title, float world_width = 8000.0f, float world_height = 8000.0f);
    void shutdown();
    ...
    void draw_model(Model* model, const glm::vec3& position, float rotation, float scale,
                    const glm::vec4& tint, float attack_tilt = 0.0f, bool enable_fog = true);
    ...
    // Camera control
    void set_camera(float x, float z);
    void set_camera_velocity(float vx, float vz);
