...
// Locations match get_vertex3d_attributes() for slot 0
struct VSInput {
    // Per-vertex attributes (slot 0) - explicit locations for Vertex3D
    [[vk::location(0)]] float3 position : POSITION;
    [[vk::location(1)]] float3 normal : NORMAL;
    [[vk::location(2)]] float2 texCoord : TEXCOORD0;
    // Per-instance attributes (slot 1, input rate = instance)
    [[vk::location(4)]] float3 instancePos : TEXCOORD1;
    [[vk::location(5)]] float instanceRotation : TEXCOORD2;
    [[vk::location(6)]] float instanceScale : TEXCOORD3;
    [[vk::location(7)]] float instanceColorVariation : TEXCOORD4;  // Matches GrassInstance in gpu_types.hpp
};
