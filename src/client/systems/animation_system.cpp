...
void update_animations(entt::registry& registry,
    ...
    const float cull_dist_sq = (cull_distance > 0.0f)
        ? (cull_distance + 50.0f) * (cull_distance + 50.0f)
        : -1.0f;

    auto update_one = [&](entt::entity entity) {
            auto&& [vel, info, combat, inst] = anim_view.get(entity);

            if (info.model_handle == mmo::engine::INVALID_MODEL_HANDLE) return;
            engine::Model* model = models.get_model(info.model_handle);
            if (!model || !model->has_skeleton) return;
            ...
            if (cull_dist_sq > 0.0f) {
                ...
                if (dx * dx + dz * dz > cull_dist_sq) return;
            }
            ...
            inst.state_machine.set_bool("attacking",
                                        combat.is_attacking || combat.current_cooldown > 0.0f);
            ...
            if (combat.is_attacking && combat.current_cooldown > 0.0f) {
                const float progress = std::min(
                    combat.current_cooldown / inst.procedural.attack_tilt_cooldown, 1.0f);
                inst.attack_tilt =
                    std::sin(progress * glm::pi<float>()) * inst.procedural.attack_tilt_max;
            }
            ...
            const glm::mat4 model_mat = engine::build_model_transform(
            ...
            if (model->foot_ik.valid && inst.procedural.foot_ik && get_terrain_height) {
                ...
                anim::apply_foot_ik(inst.player.mutable_bone_matrices(),
                                    inst.player.mutable_world_transforms(),
            ...
            if (model->foot_ik.valid && model->foot_ik.spine >= 0 && inst.procedural.lean) {
                ...
                if (horiz_speed_sq > 1.0f) {
                    forward_lean = std::min(
                        std::sqrt(horiz_speed_sq) * inst.procedural.forward_lean_factor,
                        inst.procedural.forward_lean_max);
                }
                if (smooth) {
                    lateral_lean = std::clamp(
                        -smooth->turn_rate * inst.procedural.lateral_lean_factor,
                        -inst.procedural.lateral_lean_max,
                        inst.procedural.lateral_lean_max);
                }
                anim::apply_body_lean(inst.player.mutable_bone_matrices(),
                                      inst.player.mutable_world_transforms(),
                                      model->skeleton, model->foot_ik.spine,
                                      forward_lean, lateral_lean);
            }
    };
