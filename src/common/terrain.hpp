...
inline void get_normal(float x, float z, float world_width, float world_height,
    ...
    // Match heightmap texel size (CHUNK_WORLD_SIZE / (CHUNK_RESOLUTION - 1)) for
    // consistent normals between procedural and heightmap-based calculations
    constexpr float eps = 8000.0f / 256.0f;  // ~31.25
