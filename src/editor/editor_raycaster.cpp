...
Ray EditorRaycaster::screen_to_ray(float mx, float my, int screen_w, int screen_h,
                                   const engine::scene::CameraState& camera) {
...
bool EditorRaycaster::intersect_terrain(const Ray& ray, glm::vec3& hit_pos, const TerrainHeightFn& get_height,
                                        float step_size, float max_distance) {
    // Coarse ray march. Integer counter avoids float drift on long marches.
    const int n_steps = static_cast<int>(max_distance / step_size);
...
float EditorRaycaster::intersect_aabb(const Ray& ray, const glm::vec3& aabb_min, const glm::vec3& aabb_max) {
    float tmin = -1e30f;
    float tmax = 1e30f;
