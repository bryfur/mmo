...
public:
    ...
    auto map(Fn&& fn) const& -> Result<std::invoke_result_t<Fn, const T&>, E> {
        ...
        if (is_ok()) return Result<U, E>(std::forward<Fn>(fn)(std::get<0>(data_)));
    ...
    auto map(Fn&& fn) && -> Result<std::invoke_result_t<Fn, T&&>, E> {
        ...
        if (is_ok()) return Result<U, E>(std::forward<Fn>(fn)(std::move(std::get<0>(data_))));
    ...
    auto and_then(Fn&& fn) const& -> std::invoke_result_t<Fn, const T&> {
        ...
        if (is_ok()) return std::forward<Fn>(fn)(std::get<0>(data_));
    ...
    auto and_then(Fn&& fn) && -> std::invoke_result_t<Fn, T&&> {
        ...
        if (is_ok()) return std::forward<Fn>(fn)(std::move(std::get<0>(data_)));
...
template <typename E>
inline Err<std::decay_t<E>> make_err(E&& e) {
