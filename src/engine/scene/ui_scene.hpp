 ...
 */
using UICommandData = std::variant<FilledRectCommand, RectOutlineCommand, CircleCommand, CircleOutlineCommand,
                                   LineCommand, TextCommand, ButtonCommand>;
...
public:
    ...
    void add_circle_outline(float x, float y, float radius, uint32_t color, float line_width = 2.0f, int segments = 24);
     ...
     */
    float add_text_wrapped(const std::string& text, float x, float y, float max_width, float scale = 1.0f,
                           uint32_t color = 0xFFFFFFFF, float line_height = 0.0f);

    // ========== Widget Commands ==========

    void add_button(float x, float y, float w, float h, const std::string& label, uint32_t color,
                    bool selected = false);
