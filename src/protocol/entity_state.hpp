...
struct NetEntityState : Serializable<NetEntityState> {
    ...
    static constexpr size_t serialized_size() {
        return sizeof(uint32_t) + sizeof(EntityType) + sizeof(uint8_t) * 4 + sizeof(float) * 13 + sizeof(uint32_t) +
               32 + sizeof(uint8_t) + sizeof(float) + // attack_cooldown
               32 + sizeof(float) + 16 + 16 + sizeof(float) + sizeof(uint8_t) + sizeof(uint16_t);
    }
