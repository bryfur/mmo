...
public:
    ...
    [[nodiscard]] static constexpr float distance_sq_xz(float ax, float az, float bx, float bz) noexcept {
        float dx = bx - ax;
        float dz = bz - az;
        return dx * dx + dz * dz;
    }
    ...
    [[nodiscard]] static constexpr bool in_range(float ax, float az, float bx, float bz, float range) noexcept {
        if (range <= 0.0f) {
            return false;
        }
    ...
    [[nodiscard]] static bool in_cone(float ax, float az, float bx, float bz, float dir_x, float dir_z,
                                      float cone_angle) noexcept {
        if (cone_angle <= 0.0f) {
            return true;
        }
        if (cone_angle >= 3.14159265f) {
            return true; // covers exact-behind case
        }
        float dx = bx - ax;
        float dz = bz - az;
    ...
    [[nodiscard]] static bool can_target(float ax, float az, float bx, float bz, float dir_x, float dir_z, float range,
                                         float cone_angle) noexcept {
        return in_range(ax, az, bx, bz, range) && in_cone(ax, az, bx, bz, dir_x, dir_z, cone_angle);
    }
    ...
    ///   - else 0 (no filter)
    [[nodiscard]] static constexpr float effective_cone_half_angle(float cone_angle, float spread_angle,
                                                                   bool piercing) noexcept {
        if (cone_angle > 0.0f) {
            return cone_angle;
        }
        if (spread_angle > 0.0f) {
            return spread_angle * 0.5f;
        }
        if (piercing) {
            return 0.35f;
        }
