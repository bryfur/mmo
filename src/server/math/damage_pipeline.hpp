...
public:
    ...
    [[nodiscard]] static constexpr Roll roll_with_crit(float base_damage, float crit_chance, float crit_damage_mult,
    ...
    [[nodiscard]] static constexpr float apply_mitigation(Roll roll, const Mitigation& m) noexcept {
        if (m.is_invulnerable) {
            return 0.0f;
        }
    ...
    [[nodiscard]] static constexpr ShieldAbsorbResult absorb_with_shield(float damage, float shield_value) noexcept {
    ...
    [[nodiscard]] static constexpr HealthApplyResult apply_to_health(float current_health, float /*max_health*/,
                                                                     float damage, bool has_cheat_death) noexcept {
        HealthApplyResult r;
        bool was_alive = current_health > 0.0f;
        float new_hp = current_health - damage;
        new_hp = std::max(new_hp, 0.0f);
        r.new_health = new_hp;
    ...
    [[nodiscard]] static constexpr float compute_lifesteal_heal(float damage_dealt, float lifesteal_pct) noexcept {
        if (lifesteal_pct <= 0.0f || damage_dealt <= 0.0f) {
            return 0.0f;
        }
