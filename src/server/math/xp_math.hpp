...
public:
    ...
    [[nodiscard]] static int level_floor(const Curve& curve, int level) noexcept {
        ...
        if (idx >= static_cast<int>(curve.size())) {
            idx = static_cast<int>(curve.size()) - 1;
        }
        return curve[idx];
    }
    ...
    [[nodiscard]] static int compute_new_level(const Curve& curve, int current_level, int current_xp,
    ...
    [[nodiscard]] static int compute_death_xp_loss(const Curve& curve, int current_level, int current_xp,
                                                   float loss_pct) noexcept {
    ...
    [[nodiscard]] static constexpr float level_diff_xp_modifier(int player_level, int target_level) noexcept {
