...
public:
    ...
    [[nodiscard]] static Result check(const Inputs& in) noexcept {
        if (!in.recipe_exists)                          return Result::UnknownRecipe;
        if (in.caster_level < in.recipe_required_level) return Result::LevelTooLow;
        if (in.player_gold < in.gold_cost)              return Result::NotEnoughGold;
        for (const auto& ing : in.ingredients) {
            if (ing.available < ing.required)           return Result::MissingIngredient;
        }
        if (!in.station_requirement.empty()
            && in.station_requirement != "any"
            && !in.near_matching_station) {
            return Result::NotNearStation;
        }
        if (!in.inventory_has_room_for_output)          return Result::InventoryFull;
        return Result::Ok;
    }
    ...
    [[nodiscard]] static constexpr bool near_station(
        float px, float pz, float sx, float sz) noexcept {
        float dx = px - sx;
        float dz = pz - sz;
