...
public:
    ...
    [[nodiscard]] static constexpr std::uint32_t promote_new_leader(
        std::span<const std::uint32_t> remaining_members) noexcept {
        if (remaining_members.size() < 2) return 0;
        return remaining_members.front();
    }

    /// Should a party be disbanded? Parties with one or zero members
    /// aren't parties.
    [[nodiscard]] static constexpr bool should_disband(std::size_t member_count) noexcept {
