...
public:
    ...
    [[nodiscard]] static constexpr Result check(const Inputs& in) noexcept {
        if (!in.skill_exists)                        return Result::NoSuchSkill;
        if (!in.caster_alive)                        return Result::CasterDead;
        if (in.skill_class != in.caster_class)       return Result::WrongClass;
        if (in.caster_level < in.skill_unlock_level) return Result::LevelTooLow;
        if (in.current_cooldown > 0.0f)              return Result::OnCooldown;
        if (in.caster_mana < in.mana_cost)           return Result::InsufficientMana;
        return Result::Ok;
    }

    /// Effective cooldown after talent CDR + multiplier. Floor of 0.5s
    /// prevents stacking from trivializing cooldowns entirely.
    [[nodiscard]] static constexpr float effective_cooldown(
        float base_cooldown, float cooldown_mult, float global_cdr) noexcept {
    ...
    [[nodiscard]] static constexpr float effective_mana_cost(
        float base_mana_cost, float mana_cost_mult) noexcept {
