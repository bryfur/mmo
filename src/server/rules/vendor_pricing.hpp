...
public:
    ...
    [[nodiscard]] static constexpr int unit_buy_price(int stock_override_price, int item_sell_value,
                                                      float buy_price_multiplier) noexcept {
        if (stock_override_price > 0) {
            return stock_override_price;
        }
    ...
    [[nodiscard]] static constexpr int unit_sell_price(int item_sell_value, float sell_price_multiplier) noexcept {
    ...
    [[nodiscard]] static constexpr bool within_range(float px, float pz, float nx, float nz) noexcept {
        float dx = px - nx;
        float dz = pz - nz;
