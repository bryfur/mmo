...
private:
    void RecordCollision(const JPH::Body& body1, const JPH::Body& body2,
        ...
        active_pairs_.insert(make_contact_key(event.entity_a_network_id,
                                              event.entity_b_network_id));
    }
...
void PhysicsSystem::shutdown() {
    ...
    for (auto& [entity, state] : impl_->characters) {
        ...
        if (enit == impl_->entity_to_network.end()) continue;
        auto bit = impl_->network_to_body.find(enit->second);
        if (bit == impl_->network_to_body.end()) continue;
...
void PhysicsSystem::build_terrain(const mmo::protocol::HeightmapChunk& chunk) {
    ...
    const float range = mmo::protocol::heightmap_config::MAX_HEIGHT
    ...
    JPH::HeightFieldShapeSettings settings(
        samples.data(),
        JPH::Vec3(chunk.world_origin_x, 0.0f, chunk.world_origin_z),
        JPH::Vec3(texel_size, 1.0f, texel_size),
        sample_count);
    ...
    if (result.HasError()) {
        std::cerr << "[Physics] Failed to build HeightFieldShape: "
    ...
    if (body == nullptr) {
        std::cerr << "[Physics] Failed to create terrain body" << std::endl;
        return;
    }
...
float PhysicsSystem::terrain_height(float x, float z) const {
    if (impl_->initialized && impl_->terrain_ready) {
        JPH::RRayCast ray(
            JPH::RVec3(x, mmo::protocol::heightmap_config::MAX_HEIGHT + 10.0f, z),
            JPH::Vec3(0.0f, -(mmo::protocol::heightmap_config::MAX_HEIGHT
                              - mmo::protocol::heightmap_config::MIN_HEIGHT + 20.0f), 0.0f));
...
void PhysicsSystem::create_bodies(entt::registry& registry) {
    ...
    for (auto entity : view) {
        ...
        const bool is_character =
            registry.all_of<ecs::PlayerTag>(entity) &&
        ...
        if (is_character) {
            ...
            JPH::Ref<JPH::CharacterVirtual> controller = new JPH::CharacterVirtual(
                settings, position, JPH::Quat::sIdentity(),
                static_cast<uint64_t>(network_id.id),
                impl_->physics_system.get());
            ...
            registry.emplace<ecs::PhysicsBody>(entity,
                inner.IsInvalid() ? uint32_t{0xFFFFFFFFu} : inner.GetIndexAndSequenceNumber(),
                true);
            continue;
        }
        ...
        JPH::BodyCreationSettings settings(
            shape, position, JPH::Quat::sIdentity(), motion_type, layer);
...
void PhysicsSystem::update(entt::registry& registry, float fixed_dt) {
        ...
        if (char_it != impl_->characters.end()) {
            ...
            if (it != impl_->network_to_body.end()) {
                ...
                body_interface.SetPosition(it->second,
                    JPH::RVec3(transform.x, transform.y + offset_y, transform.z),
    ...
    for (auto entity : dynamic_view) {
        ...
        auto it = impl_->network_to_body.find(network_id.id);
        if (it == impl_->network_to_body.end()) continue;
    ...
    for (auto entity : kinematic_view) {
        ...
        auto it = impl_->network_to_body.find(network_id->id);
        if (it != impl_->network_to_body.end()) {
            body_interface.MoveKinematic(it->second,
                JPH::RVec3(transform.x, transform.y, transform.z),
    ...
    // Drive CharacterVirtuals from ECS velocity and step them.
    {
        ...
        for (auto& [entity, state] : impl_->characters) {
            ...
            state.controller->ExtendedUpdate(fixed_dt,
                JPH::Vec3(0.0f, state.gravity_y, 0.0f),
                upd, bp_filter, obj_filter, body_filter, shape_filter,
                *impl_->temp_allocator);
        }
    }

    const int collision_steps = 1;
    impl_->physics_system->Update(fixed_dt, collision_steps,
    ...
    if (impl_->collision_callback) {
        ...
        for (const auto& event : events) {
            ...
            if (a_it != impl_->network_to_entity.end() &&
                b_it != impl_->network_to_entity.end()) {
...
bool PhysicsSystem::overlap_sphere(const glm::vec3& center, float radius,
                                   std::vector<entt::entity>& out) const {
    out.clear();
    if (!impl_->initialized || radius <= 0.0f) return false;
    ...
    impl_->physics_system->GetNarrowPhaseQuery().CollideShape(
        sphere, JPH::Vec3::sReplicate(1.0f), xform, settings,
    ...
    for (const auto& h : collector.mHits) {
        ...
        if (body_it == impl_->body_to_network.end()) continue;
        if (!seen.insert(body_it->second).second) continue;
...
void PhysicsSystem::update_body_shape(entt::registry& registry, entt::entity entity,
    ...
    if (char_it != impl_->characters.end()) {
        ...
        JPH::BodyFilter body_filter;
        JPH::ShapeFilter shape_filter;
        char_it->second.controller->SetShape(shape, 1.5f * char_it->second.controller->GetCharacterPadding(),
            bp_filter, obj_filter, body_filter, shape_filter, *impl_->temp_allocator);
        return;
    }
