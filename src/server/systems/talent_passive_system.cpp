...
void update_talent_passives(entt::registry& registry, float dt, const GameConfig& config) {
    ...
    for (auto entity : view) {
        ...
        if (tr.cheat_death_timer > 0.0f) {
            tr.cheat_death_timer = std::max(0.0f, tr.cheat_death_timer - dt);
        }
        if (tr.shield_regen_timer > 0.0f) {
            tr.shield_regen_timer = std::max(0.0f, tr.shield_regen_timer - dt);
        }
        if (tr.panic_freeze_timer > 0.0f) {
            tr.panic_freeze_timer = std::max(0.0f, tr.panic_freeze_timer - dt);
        }
        ...
        if (tp.shield_regen_pct > 0.0f && tr.shield_regen_timer <= 0.0f) {
            ...
            apply_effect(registry, entity,
                         ecs::make_status_effect(ecs::StatusEffect::Type::Shield, tp.shield_regen_cooldown_max + 0.1f,
                                                 shield_value));
        ...
        if (tp.fury_threshold > 0.0f && health.ratio() <= tp.fury_threshold) {
            ...
            if (tp.fury_attack_speed_mult > 1.0f) {
                apply_effect(registry, entity,
                             ecs::make_status_effect(ecs::StatusEffect::Type::SpeedBoost, 0.5f,
                                                     tp.fury_attack_speed_mult - 1.0f, 1));
            }
        }

        // Panic freeze at low HP (Frozen Heart)
        if (tp.panic_freeze_radius > 0.0f && tp.panic_freeze_threshold > 0.0f && tr.panic_freeze_timer <= 0.0f &&
            health.ratio() < tp.panic_freeze_threshold) {
            ...
            for (auto npc : npc_view) {
                ...
                float dx = nt.x - transform.x;
                float dz = nt.z - transform.z;
        ...
        if (tp.nearby_debuff_range > 0.0f && tp.nearby_damage_reduction > 0.0f) {
            ...
            if (tr.debuff_aura_timer <= 0.0f) {
                ...
                uint32_t player_net_id =
                    registry.all_of<ecs::NetworkId>(entity) ? registry.get<ecs::NetworkId>(entity).id : 0;
                ...
                for (auto npc : npc_view3) {
                    ...
                    if (std::sqrt(dx * dx + dz * dz) <= tp.nearby_debuff_range) {
                        ...
                        apply_effect(registry, npc,
                                     ecs::make_status_effect(ecs::StatusEffect::Type::DamageBoost, 1.1f,
                                                             -tp.nearby_damage_reduction, player_net_id));
                    }
                }
            }
        }
