...
ZoneSystem::ZoneSystem(const GameConfig& config) : config_(&config), rng_(std::random_device{}()) {
    for (const auto& zone : config_->zones()) {
        if (zone.monster_density <= 0.0f) {
            continue;
        }
        if (zone.monster_types.empty()) {
            continue;
        }
...
void ZoneSystem::spawn_initial_monsters(entt::registry& registry, std::function<uint32_t()> next_id_fn,
    ...
    for (const auto& spawn : zone_spawns_) {
        ...
        for (int i = 0; i < spawn.target_count; ++i) {
            ...
            registry.emplace<ecs::MonsterTypeId>(entity,
                                                 ecs::MonsterTypeId{mt->id, mt->level, mt->xp_reward, mt->gold_reward});
...
void ZoneSystem::respawn_monster(entt::registry& registry, entt::entity monster,
    ...
    if (!zone || zone->monster_types.empty()) {
        return;
    }
